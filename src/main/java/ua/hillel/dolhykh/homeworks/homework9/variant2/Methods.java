...
public class Methods {
    ...
    public static List<WordOccurence> findOccurrence(List<String> words) {
        List<String> uniqueWords = new ArrayList<>();
        List<WordOccurence> wordOccurrences = new ArrayList<>();
        for (int i = 0; i < words.size(); i++) {
            String wordFromList = words.get(i);
            int counter = 0;
            for (int j = 0; j < words.size(); j++) {
            ...
            if (!uniqueWords.contains(wordFromList)) {
                uniqueWords.add(wordFromList);
                wordOccurrences.add(new WordOccurence(wordFromList, counter));
            }
        }
